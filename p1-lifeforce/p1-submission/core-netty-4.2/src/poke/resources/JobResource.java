			...
			if (request != null && request.getBody().hasPhotoPayload()) {
				...
				case read:
					try {
						...
						BlobStorageProfile blobFound = BlobStorageServiceImplementation
								.getInstance().findByUuid(
										request.getBody().getPhotoPayload()
												.getUuid());
				...
				case delete:
					...
					try {
						...
						Boolean blobSuccess = BlobStorageServiceImplementation
								.getInstance().deleteBlobStorageByUuid(
										request.getBody().getPhotoPayload()
												.getUuid());
